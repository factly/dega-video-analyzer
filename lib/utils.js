 ...
 * @param {int} input_delay_time  - delay in milliseconds between try, default 30 seconds
 * @param {object} fn function to execute
 * @param {object} logger - logger
 * @returns {object} - returns result of function
 */
utils.retry = function retry(maxRetries, input_delay_time , fn, logger) {
    var delay_time = input_delay_time || 30000;
    return fn()
        .then((response) => {
        ...
        .catch((err) => {
            if (maxRetries === 0) {
                ...
                logger.info(`Trying to connect with a delay of ${delay_time } millis, attempt #${5 - currentAttempt}`);
                return delay(delay_time)
                    .then(() => retry(maxRetries - 1, delay_time , fn, logger));
            }
        });
};
